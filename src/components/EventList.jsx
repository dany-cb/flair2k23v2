...
export default function Events() {
  const elist = [
    ...
    },
    {
      name: "Stellar Hunt",
      description:
        "The future of humanity lies in your hands. A rogue faction is sabotaging the mission to find a new habitable planet for our species. It is your destiny to aid Cooper and his team in ensuring the success of the Interstellar mission.",
    },
  ];
  ...
  return (
    <>
      ...
      <section className="w-full -mt-[18%]" id="events">
        {elist.map((title, index) => (
          ...
          >
            ...
            >
              <Link
                href={`/events/${title.name.toLowerCase().replace(/ /g, "-")}`}
              >
                <span className="flex gap-2 underline">
                  <RandoText text={title.name} delay={0.7} />
                </span>
              </Link>
              <div className="text-xl w-[75%] lg:w-1/2 font-serif font-normal mt-5 tracking-normal leading-tight">
                {title.description}
              </div>
            </motion.div>
          </div>
